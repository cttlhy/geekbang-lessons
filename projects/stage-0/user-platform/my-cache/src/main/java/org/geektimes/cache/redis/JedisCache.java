...
public class JedisCache<K extends Serializable, V extends Serializable> extends AbstractCache<K, V> {
    ...
    public JedisCache(CacheManager cacheManager, String cacheName,
        ...
        this.jedis = jedis;
        this.serializer = (CacheSerializer) new ByteArraySerializer();
    }
...
//        byte[] keyBytes = serialize(key);
    ...
    protected Set<K> keySet() {
        ...
        Set<K> collect = new HashSet<>();
        keys.forEach(
                k->{
                    K deserialize = null;
                    try {
                        deserialize = (K) serializer.deserialize(k);
                    } catch (CacheException e) {
                        deserialize = (K) new String(k,StandardCharsets.UTF_8);
                    }
